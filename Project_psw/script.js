        ...
        fetch('get_order.php') 
            ...
            .then(orders => {
                ...
                if (!orders || orders.length === 0) {
                    if (tableBody) {
                        tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Belum ada data pesanan masuk.</td></tr>';
                    }
                    return;
                }
                ...
                orders.forEach(order => {
                    ...
                    tableHtml += `
                        <tr>
                            ...
                            <td><span class="status-badge" style="background:#e8f5e9; color:#2e7d32; padding:5px 10px; border-radius:5px; font-size:12px;">Success</span></td>
                        </tr>
                    `;
                });
            ...
            .catch(error => {
                const tableBody = document.getElementById('order-table-body');
                if (tableBody) {
                    tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:red;">Gagal memuat data dari database.</td></tr>';
                }
            });
    }
...
function payNow() {
    ...
    .then(text => {
        try {
            ...
            throw new Error("Gagal membaca respon server. Silakan buka Inspect Element -> tab Console untuk melihat error database.");
        }
    })
    .then(data => {
        if (data.status === 'success') {
